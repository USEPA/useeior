...
```{r Validate 1: results using import factors + domestic matrices}
...
#  y_d <- rowSums(model$U_d[1:73,72:91])
...
```{r Validate 2: results using import factors + domestic matrices using y as a matrix rather than a vector}
...
```{r Validate 3: results using diagonalized y_d and y_m rather than a vector}
