...
#' @description Generate direct requirements Use table for 2 region models using domestic Use table with trade data generated by stateior
#' @param model An EEIO form USEEIO model object with model specs and IO meta data loaded.
...
generate2RDirectRequirementsfromUseWithTrade <- function(model){
  ...
  #additional code not in validate2RegionLagainstOutput function to create normalized use table
  U_d_n_w_trade <- cbind(rbind(SoI2SoI_A, RoUS2SoI_A), rbind(SoI2RoUS_A, RoUS2RoUS_A))
  colnames(U_d_n_w_trade) <- colnames(model$U_n)
  rownames(U_d_n_w_trade) <- rownames(model$U_n)
  return(U_d_n_w_trade)
